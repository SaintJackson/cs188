...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for jointVar in eliminationOrder:
            currentFactors, jointFactor = joinFactorsByVariable(currentFactors, jointVar)
            if len(jointFactor.unconditionedVariables()) > 1:
                jointFactor = eliminate(jointFactor, jointVar)
                currentFactors.append(jointFactor)
