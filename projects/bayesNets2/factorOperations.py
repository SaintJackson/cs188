...
def joinFactors(factors):
    ...
    for f in factors:
        ...
        unconditioned.extend(unconditionedVars)
        for conditionedVar in conditionedVars:
            if conditionedVar not in conditioned:
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        prob = reduce(lambda x, y: x * y, map(lambda factor: factor.getProbability(assignment), factors))
...
def normalize(factor):
    ...
    proSum = 0
    old_assginment = factor.getAllPossibleAssignmentDicts()
    for assignment in old_assginment:
        proSum +=factor.getProbability(assignment)
    if proSum == assignment:
        return None

    newFactor = Factor(unconditionedVars, conditionedVars, variableDomainsDict)

    for assignment in newFactor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment) / proSum
        newFactor.setProbability(assignment, prob)
    return newFactor
