...
def fillObsCPT(bayesNet, gameState):
    ...
    posOfDict[BOTTOM_LEFT_VAL] = bottomLeftPos;
    posOfDict[TOP_LEFT_VAL] = topLeftPos
    posOfDict[BOTTOM_RIGHT_VAL] = bottomRightPos
    posOfDict[TOP_RIGHT_VAL] = topRightPos

    def isAdjacent(obsPos, ghostPos):
        return abs(obsPos[0] - ghostPos[0]) <= 1 and abs(obsPos[1] - ghostPos[1]) <= 1
    ...
    for housePos in house_locations:
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsCPT:
                foodPos = posOfDict[assignment[FOOD_HOUSE_VAR]]
                ghostPos = posOfDict[assignment[GHOST_HOUSE_VAR]]
                color = assignment[obsVar]

                if isAdjacent(obsPos, foodPos):
                    if color == RED_OBS_VAL:
                        ...
                        prob = 0
                elif isAdjacent(obsPos, ghostPos):
                    if color == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif color == BLUE_OBS_VAL:
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        factor = inference.inferenceByVariableElimination(self.bayesNet, [HOUSE_VARS], evidence,
                                                          eliminationOrder)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, explEvidence in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob * max(self.computeEnterValues(explEvidence, enterEliminationOrder))
