...
## Build a helper function: general_search()
def general_search(problem, fringe, add_to_fringe):
    closedSet = set()
    start = (problem.getStartState(), 0, []) 
    add_to_fringe(fringe, start, 0)
    ...
    while not fringe.isEmpty():
        ...
        if not node in closedSet:
            ...
            for childNode, childAction, childCost in problem.getSuccessors(node):
                new_path = path + [childAction] ## concate list to use [].
                new_cost = cost + childCost
                new_state = (childNode, new_cost, new_path)
                add_to_fringe(fringe, new_state, new_cost)
...
def depthFirstSearch(problem):
    ...
    """
    fringe = util.Stack()
    def add_to_fringe(fringe, state, cost):
        fringe.push(state)
    return general_search(problem, fringe, add_to_fringe)
