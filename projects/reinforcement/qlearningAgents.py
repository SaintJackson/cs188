...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        found = util.flipCoin(self.epsilon)
        if found:
    ...
    def update(self, state, action, nextState, reward):
        ...
        rewardPart = self.alpha * (reward + self.discount * maxQnext)
        if not nextState:
            self.Qvalues[(state, action)] = oldPart + self.alpha * reward
        else:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature, value in self.featExtractor.getFeatures(state, action).items(): 
            total += self.getWeights()[feature] * value
        return total
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        maxNextQ = self.getQValue(nextState, self.getPolicy(nextState))
        Qvalue = self.getQValue(state, action)
        diff = (reward + self.discount * maxNextQ) - Qvalue
