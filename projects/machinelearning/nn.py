...
class Graph(object):
    ...
    def backprop(self):
        ...
        #reverse the list so as to backpropogation gradients
        for node in self.nodeslist[::-1]:
            parentGradient = node.backward(self.get_inputs(node), node.gradient)
            for n in range(len(node.get_parents())):
                node.get_parents()[n].gradient += parentGradient[n]
    ...
    def step(self, step_size):
        ...
        for var in self.variables:
            ...
            var.data = var.data - step_size * var_grad
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        average = 0.5 * np.power((inputs[0] - inputs[1]), 2)
    ...
    def backward(inputs, gradient):
        ...
        forward1 = (inputs[0] - inputs[1]) * gradient / inputs[0].size
        forward2 = -(inputs[0] - inputs[1]) * gradient / inputs[0].size
