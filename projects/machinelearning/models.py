...
class RegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        mulLayer = nn.MatrixVectorAdd(graph, mul2, self.b2)
        ...
        if y is not None:
            ...
            loss = nn.SquareLoss(graph, mulLayer, input_y)
            return graph
        else:
            ...
            return graph.get_output(mulLayer)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        # assume hidden layers are 500
        self.w1 = nn.Variable(784, 500)
        self.b = nn.Variable(500)
...
class DeepQModel(Model):
    ...
    def __init__(self):
        ...
        self.b = nn.Variable(hidden_layers)
        self.w2 = nn.Variable(hidden_layers,2)
    ...
    def run(self, states, Q_target=None):
        ...
        add1 = nn.MatrixVectorAdd(graph, mul1, self.b)
        hidden_output = nn.ReLU(graph, add1)
        mul2 = nn.MatrixMultiply(graph, hidden_output, self.w2)
        add2 = nn.MatrixVectorAdd(graph, mul2, self.b1)
        ...
        if Q_target is not None:
            ...
            loss = nn.SquareLoss(graph, add2, input_q_target)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(add2)
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        hidden_layers = 400
        self.b1 = nn.Variable(hidden_layers)
        self.w1 = nn.Variable(200, hidden_layers)
        self.b2 = nn.Variable(200)
        self.w2 = nn.Variable(hidden_layers, 200)
        self.b3 = nn.Variable(5)
        self.w3 = nn.Variable(200, 5)
        
        self.add0 = nn.Variable(200)
        self.c = nn.Variable(47, 200)
    ...
    def run(self, xs, y=None):
        ...
        "* YOUR CODE HERE *"
        graph = nn.Graph([self.b1, self.b2, self.b3, self.w1, self.w2, self.w3, self.add0, self.c])
